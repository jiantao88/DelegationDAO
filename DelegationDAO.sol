...
contract DelegationDAO is AccessControl {
    ...
    mapping(uint => mapping (address => bool)) private proposalVotes;

    event ProposalAdd(daoState state);

    function startRevokeProposal(uint voter) public {
        Proposal memory proprosal;
        proprosal.state = daoState.REVOKED;
        proprosal.totalVoter = voter;
        proposals[proposalNumber] = proprosal;
        emit ProposalAdd(daoState.REVOKED);
    }

    function startResetProposal(uint voter) public {
        Proposal memory proposal;
        proposal.state = daoState.COLLECTING;
        proposal.totalVoter = voter;
        proposals[proposalNumber] = proposal;
        emit ProposalAdd(daoState.COLLECTING);
    }

    function voteYes(uint i) public {
        require(proposals[i].success == false, "already success");
        require(proposalVotes[i][msg.sender] == false, "already vote");
        proposalVotes[i][msg.sender] = true;
        proposals[i].approveVote += 1;
        if (proposals[i].approveVote * 100 / proposals[i].totalVoter > 50) {
            proposals[i].success = true;
            execute(proposals[i]);
        }
    }

    function execute(Proposal memory proposal) internal {
        if (proposal.state == daoState.STAKING) {
